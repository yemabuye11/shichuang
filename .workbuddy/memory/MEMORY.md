...
- ⏳ **遇到 API/模型限流（429）不许停工、不许让用户等**（2026-09-15 野马明确要求，原话："如果遇到限流，请设定任务，限流结束后继续跑任务，以后都这样"）。
  正确做法：① 把未完成部分写成明确任务（记录当前进度、剩哪些文件/步骤）；② 用 `automation_update` 建**一次性定时任务**，设定在限流窗口结束后（通常几分钟~几小时）自动继续执行；③ 主动告知用户"已排好 X 点自动续跑"，而不是干等或让用户重试。
- **新功能先出大纲给野马拍板再动手**（2026-09-15 要求）。涉及合规/定价的功能尤其如此。
