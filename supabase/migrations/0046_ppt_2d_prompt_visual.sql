...
--
-- 本次改动（只动这两个 key）：
--   1. 必须有图，且**按学科分两档**（提示词内给了判据，让模型自己归类：本节主题主要是
--      「数据/函数/实验/统计/趋势/占比/对比/图形」还是「文本解读/脉络梳理/观点论述/语言运用」）：
--      - **数据型档**（数学 / 物理 / 化学 / 生物 / 地理 / 信息技术，及任何涉及数据、函数、
--        实验、统计、趋势、占比、对比的主题）：除封面、目录外**每一页 / 每一章**都有图；
--        PPT 12 页至少 10 页带图，不许连续 3 页纯文字；2D 不允许连续 2 个无图章节。
--      - **文科档**（语文 / 历史 / 政治 / 道德与法治 / 英语，及以文本解读、脉络梳理、
--        观点论述为主的主题）：**每 2 页 / 每 2 章至少 1 张图**，整份 **≥ 3 张**，
--        仍**禁止整份 0 张图**；鼓励用 `image` data URI 画文章脉络图 / 情节推进图 /
--        时间轴 / 人物关系图 / 论证结构图，但**禁止没有信息量的装饰图凑数**。
--      分档原因：文科硬凑关系图会直接拉低"可直接对外展示"的观感，
--      宁可规则细一点，也不要出现无意义图表。
--   2. **跨科兜底**：任何学科、任何一页 / 一章，若确实无图可画，**必须**用 `table` 或
--      `list` 把核心信息结构化表达出来；不许留白、不许只写散文、不许写"建议配图"。
--      （避免模型在"没图可画"时直接交白卷。）
--   3. 图表优先：能用图表表达的数据与关系（函数、趋势、占比、分类对比、多组比较）
--      **必须**用 chart 块；**不许**把图表退化成文字表格。
--   4. 放开 image 块：src 允许且只允许内联 SVG 的 data URI（给出可照抄模板 + 编码规则）；
--      **禁止外链**（教室常断网必挂，也会被浏览器安全策略拦截）；
--      **禁止 src 留空**（渲染出来只有虚线框）。
--   5. 质量下限：PPT **≥ 12 页**、2D 课件 **≥ 6 个章节**；每页 / 每章必须有实质内容；
...
问自己一句：**这节课的主题，主要是「数据 / 函数 / 实验 / 统计 / 趋势 / 占比 / 对比 /
图形」，还是「文本解读 / 脉络梳理 / 观点论述 / 语言运用」？**

- **答"前者" → 数据型档**：典型学科是数学、物理、化学、生物、地理、信息技术；
  也包括**任何**涉及数据、函数、实验、统计、趋势、占比、对比的主题
  （例如历史课的"人口迁移数据统计"、政治课的"居民收入占比变化"都算这一档）。
- **答"后者" → 文科档**：典型学科是语文、历史、政治、道德与法治、英语；
  以及以文本解读、脉络梳理、观点论述、语言运用为主的主题。

### 数据型档：除封面 / 目录外，**每一页**都有图

- **除封面页与目录页外，每一页 `body` 里都必须至少有一个 `chart` 或 `image` 块，
  全是文字块的页面判定为不合格页。**
- 一份 12 页的课件，至少 **10 页**带图（只有封面 + 目录允许无图）。
- 教师把课件投到教室大屏上时，**不允许出现连续 3 页纯文字**。

### 文科档：每 2 页至少 1 张图，整份 ≥ 3 张

- **每 2 页至少 1 张图**（`chart` 或 `image` 合计），整份课件 **≥ 3 张图**。
- 不看带图页的比例，但**仍然禁止"整份一张图都没有"**。
- 文科里**有意义、不算凑数**的图（用 `image` 块的 data URI 画）：
  **文章脉络图 / 情节推进图 / 时间轴 / 人物关系图 / 论证结构图 / 知识结构示意图**。
  这些对文科是真实有用的，请放心使用。
- **不要**为了凑数去画没有信息量的装饰图（例如把"段落大意"画成一个圆、
  把"中心思想"画成一朵云）。**没有信息量的图比没有图更糟。**

### 跨科兜底：没有图的一页，必须把信息结构化

**任何学科、任何一页**，如果确实没有合适的图可画，
**必须**用 `table` 或 `list` 把该页的核心信息结构化表达出来
（对比表、要素清单、步骤流程、要点层级）。
**不许留白、不许只写一段没有结构的散文、不许写"建议配图：XX"。**
（这不与下面的「图表优先」冲突：能画成图的仍然必须画成 `chart`；
这条兜底只用于**确实画不出图**的页。）
...
| 带图页（数据型档） | 除封面 / 目录外，**每一页**都至少有 1 个 `chart` 或 `image` 块 |
| 带图页（文科档） | **每 2 页至少 1 张**，整份 **≥ 3 张**；**不允许整份 0 张** |
| `chart` 总数 | 数据型学科 **≥ 3 个**；文科档不做硬性要求（能用 `chart` 更好，否则用 `image` 示意图凑够张数） |
| 无图页兜底 | 任何无图的一页都必须有 `table` 或 `list` 承载该页核心信息 |
...
其中**每个知识讲解页、每个例题页都必须带图**（文科档按"每 2 页 1 张"执行，
但在张数不够时优先给这些页配图）。
...
- **整份课件一张图都没有**（任何学科都不允许）；
- 数据型档出现**纯文字页**（封面 / 目录除外）、或出现**连续 3 页纯文字**；
- 文科档整份**不足 3 张图**、或出现连续 4 页无图；
- 无图页既没有 `table` / `list` 结构、又只写了一段没有结构的散文；
- 「略」「同上」「……」「见课件」「此处插入图片 / 表格 / 视频」「此处可展开」
  「教师可自行补充」「学生自行阅读课本」等占位与施工标记；
- 为了凑数而画的**没有信息量的装饰图**（把抽象概念画成圆、云、箭头堆砌等）；
- 把本该是图表的内容写成文字表格；
...
- **每一个章节（由一个 `heading` 且 `level=2` 引领的小节）都必须至少有一个
  `chart` 或 `image` 块，整节只有文字的章节判定为不合格。**
- 一份 6 章节的课件，至少 **6 个**图（每章 ≥ 1 个）。
- **不允许出现连续 2 个没有图的章节。**

### 文科档：每 2 章至少 1 张图，整份 ≥ 3 张

- **每 2 个章节至少 1 张图**（`chart` 或 `image` 合计），整份课件 **≥ 3 张图**。
- 不看带图章节的比例，但**仍然禁止"整份一张图都没有"**。
- 文科里**有意义、不算凑数**的图（用 `image` 块的 data URI 画）：
  **文章脉络图 / 情节推进图 / 时间轴 / 人物关系图 / 论证结构图 / 知识结构示意图**。
  这些对文科是真实有用的，请放心使用。
- **不要**为了凑数去画没有信息量的装饰图（例如把"段落大意"画成一个圆、
...
### 跨科兜底：没有图的章节，必须把信息结构化

**任何学科、任何一章**，如果确实没有合适的图可画，
**必须**用 `table` 或 `list` 把该章的核心信息结构化表达出来
（对比表、要素清单、步骤流程、要点层级）。
**不许留白、不许只写一段没有结构的散文、不许写"建议配图：XX"。**
（这不与下面的「图表优先」冲突：能画成图的仍然必须画成 `chart`；
这条兜底只用于**确实画不出图**的章。）
...
| 章节配图（数据型档） | 每一章 **≥ 1 个** `chart` 或 `image` 块 |
| 章节配图（文科档） | **每 2 章至少 1 张**，整份 **≥ 3 张**；**不允许整份 0 张** |
| `chart` 总数 | 数据型学科 **≥ 2 个**，函数 / 数据 / 对比型内容必须出 `chart`；文科档不做硬性要求（能用 `chart` 更好，否则用 `image` 示意图凑够张数） |
| 无图章兜底 | 任何无图的章节都必须有 `table` 或 `list` 承载该章核心信息 |
...
2. **≥ 6 个章节**，每章 = `paragraph` 讲解 → `chart` / `image` 出图 → `callout` 提示易错点
   （文科档按"每 2 章 1 张"执行；无图的章节必须有 `table` 或 `list` 兜底）；
...
- **整份课件一张图都没有**（任何学科都不允许）；
- 数据型档出现**无图章节**、或出现连续 2 个无图章节；
- 文科档整份**不足 3 张图**、或出现连续 4 个章节无图；
- 无图章节既没有 `table` / `list` 结构、又只写了一段没有结构的散文；
...
--
-- 1. 确认 PPT 提示词已生效（重点看学科分档那段在不在）：
--      select key, version, is_active, content
--        from public.prompt_templates
--       where key = 'doc_type:ppt' and is_active;
--    期望：命中 1 行；content 中含
--      「先判断本节属于哪一档」「数据型档」「文科档」
--      「数据型档：除封面 / 目录外，**每一页**都有图」「12 页的课件，至少 10 页带图」
--      「文科档：每 2 页至少 1 张图，整份 ≥ 3 张」「仍然禁止"整份一张图都没有"」
--      「跨科兜底：没有图的一页，必须把信息结构化」
--      「文章脉络图 / 情节推进图 / 时间轴 / 人物关系图 / 论证结构图」
--      「≥ 12 页」「data:image/svg+xml」「禁止外链」「禁止 `src` 留空」
--
-- 2. 确认 2D 课件提示词已生效：
--      select key, version, is_active, content
--        from public.prompt_templates
...
--    期望：命中 1 行；content 中含
--      「先判断本节属于哪一档」「数据型档：**每一章**都有图」
--      「文科档：每 2 章至少 1 张图，整份 ≥ 3 张」
--      「跨科兜底：没有图的章节，必须把信息结构化」
--      「≥ 6 个章节」「data:image/svg+xml」
--
-- 2b. 分档规则速查（用一条 SQL 同时确认两份提示词都有分档，应各返回 1 行）：
