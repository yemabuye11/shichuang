...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  //      单 IP 1h ≤ 100 次 —— 一开始是 3 次，但学校/教研组都走同一个公网 IP（NAT），
  //      3 次会导致一个办公室一小时内只能注册 3 个人，直接卡死试点推广。
  //      防薅羊毛主要靠上面的「单邮箱 5 次」，IP 这层只是兜底拦脚本批量刷，
  //      所以放到 100 次：正常全校/全区老师同时注册都够用。
  if ((byEmail.count ?? 0) >= 5 || (byIp.count ?? 0) >= 100) {
