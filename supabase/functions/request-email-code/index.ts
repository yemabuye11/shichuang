 ...
 * 流程：
 * 1. 校验邮箱格式；
 * 2. 限流：卡邮箱（1h ≤ 5 次），不卡 IP（1h ≤ 1000 次，实际关闭，因学校共用公网 IP）；
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // 限流：卡邮箱，不卡 IP。
  //   单邮箱 1h ≤ 5 次 —— 防同一邮箱被反复刷（主闸门）。
  //   单 IP   1h ≤ 1000 次 —— 学校/教研组几百人共用一个公网 IP（NAT），
  //       卡 IP 等于卡死全校注册，所以这一档实际关闭，只拦脚本失控刷库。
  //   注：「一个邮箱只能注册一次」由 Supabase auth 的邮箱唯一约束保证，
  //       重复注册会直接失败，不会重复发注册积分，无需在此拦截
  //       （若按 consumed 记录拦截，会导致"验证完但注册没走完"的邮箱被永久锁死）。
  if ((byEmail.count ?? 0) >= 5 || (byIp.count ?? 0) >= 1000) {
